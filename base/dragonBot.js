...
class DragonBot extends Client {
  constructor(options) {
    ...
    this.creator = new SlashCreator(this.setting["creatorSetting"]).withServer(
      new GatewayServer((handler) => this.ws.on("INTERACTION_CREATE", handler)),
    );
  }
  ...
  async init() {
    ...
    Object.keys(this.books).forEach((msgId) => {
      this.books[msgId] = new Set(this.books[msgId]);
    });
  ...
  loadSlash(category = null) {
    const results = [];
    glob
      .sync(`${this.folder["slashCommand"]}/${category ?? "*"}/*.js`)
      .forEach((file) => {
        const fileName = file.split("/").pop();
        try {
          const command = this.creator.commands.find((cmd) =>
            cmd.filePath.includes(fileName),
          );
          ...
          this.logger.error(
            `Failed to load slash-command ${fileName}( ${error} )`,
          );
