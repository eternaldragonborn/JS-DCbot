...
module.exports = class Webhooks extends SlashCommand {
    ...
    async run(ctx) {
        ...
        const hook = (await channel.fetchWebhooks()).find(v => { return v.owner?.id === process.env.APP_ID });
        switch (subcommand) {
            case 'get':
                if (hook) {
                    await ctx.send({ content: hook.id, ephemeral: true });
                } else {
                    channel.createWebhook(options.name ?? "Dragon's webhook")
                        .then(webhook => ctx.send({ content: `created\n${channel.name} : ${webhook.id}`, ephemeral: true }))
                        .catch(err => ctx.send({ content: `failed, ${err.message}`, ephemeral: true }))
                }
                break;
            case 'delete':
                if (!hook) await ctx.send({ content: '該頻道無Webhook', ephemeral: true });
                else {
                    hook.delete()
                        .then(() => ctx.send({ content: '已刪除', ephemeral: true }))
                        .catch(err => { ctx.send({ content: '失敗，' + err.message, ephemeral: true }) });
                }
                break;
            case 'edit':
                if (!hook) await ctx.send({ content: '該頻道無Webhook', ephemeral: true });
