    ...
    client.creator
        .on('commandRegister', (command, creator) => {
            command.guildIDs?.length > 0 ?
                ...
                client.logger.warn(`Global command \`${command.commandName}\` registered.`);
        })
        ...
        .on('commandError', (command, err, ctx) => {
            client.logger.error(`Command \`${command.commandName}\` execution error: ${err}`);
        ...
        .on('commandRun', (command, result, ctx) => {
            ...
            result.then(() => client.logger.trace(`Command \`${command.commandName}\` executed.`));
        })
        .on('error', (err) => {
